...
export default class UserFeedRecentActivity extends React.Component {
  ...
  getLastSevenDays(workoutType) {
    let bike = this.props.workout.filter(act => act.workout_type === workoutType);
  ...
  getElevation(workout_elevation) {
    let elevation = JSON.parse(workout_elevation)
  ...
  makePanes() {
    ...
    this.props.workout.forEach(act => {
      switch (act.workout_type) {
        case "Bike":
  ...
  render() {
    ...
    return (
      <div className="recent-workout-tab">
